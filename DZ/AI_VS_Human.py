...
while True:
    ...
    if end > 0:
        ...
        canvas.create_text(xw / 2, yw / 5 * 3, text=str(round(end / turn_time * 100)) + "%", fill="#60FF60",
                           font=("Areal", xw // 30))
        canvas.create_rectangle(xw / 2 - yw / 2 + 10, yw / 7 * 5, xw / 2 + yw / 2 - 10, yw / 7 * 5.5, width=5,
                                outline="#40A040")
        canvas.create_rectangle(xw / 2 - yw / 2 + 12, yw / 7 * 5 + 2,
                                xw / 2 - yw / 2 + 12 + (yw - 24) / turn_time * end, yw / 7 * 5.5 - 2, width=1,
                                outline="#109010", fill="#40F040")
    canvas.update()

    if x != "" and y != "" and x.isalnum() and y.isalnum() and 0 <= int(x) <= 2 and 0 <= int(y) <= 2 and field[int(y)][
        int(x)] == "0":
        your_turn = 0
        x = int(x)
        y = int(y)
        in_field(x, y, "1")
        if field[0] == "111" or field[1] == "111" or field[2] == "111" or (
        ...
        if end == 0:
            ...
            if field[0] == "222" or field[1] == "222" or field[2] == "222" or (
